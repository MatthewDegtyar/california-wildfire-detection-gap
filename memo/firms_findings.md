...
**v2 (+ elevation) and v1 are statistically indistinguishable on aggregate metrics, and v1 is the better-calibrated model in the operational band.** Brier 0.0367 ± 0.0017 vs 0.0381 ± 0.0012 — a 0.0014 gap with ±1σ bands that overlap. On calibration in the 10–20% predicted-probability bin (where many operational decisions live): **v1 under-predicts** by 1.2 pp (pred 0.140 vs actual 0.128, safer direction for an alerting product), while **v2 over-predicts** by 4.3 pp (pred 0.137 vs actual 0.094, the unsafe direction — model says fire more likely caught than it actually is). For a *detection* product, the safer direction matters more than 0.0014 in aggregate Brier. We keep v2 nominal in the headline tables for continuity with the earlier draft, but a reader picking a model to actually deploy should prefer v1 on calibration grounds; the elevation feature's contribution is a wash on the metric and a step backwards on safety in the band that matters most. (We avoid the otherwise-tempting argument "v2's gap surface looks more terrain-shaped, therefore v2 is better." A model can produce more spatially-structured output because it is picking up real signal *or* because it is overfitting to elevation; the held-out metric does not distinguish those, so neither do we.)
...
![Reliability diagram, 10 bins on OOF predictions, all four calibrated models. The operational-band detail behind the v1-vs-v2 calibration discussion in the body: in the 10--20% predicted-probability bin, GBM v1 (red) *under*-predicts by 1.2 pp (pred 0.140, actual 0.128 — the safer direction for a detection product); GBM v2 (orange) *over*-predicts by 4.3 pp in the same bin (pred 0.137, actual 0.094 — the unsafe direction, model says fire more likely caught than it actually is). Both v1 and v2 under-predict in the 30--60% bins by 8--16 pp (safer side, but data-limited: n ≤ 90 per bin). The 0--10% bin (where ~90% of fires live) is well-calibrated to within 1 pp for all four models. Bins above 70% have ≤35 fires each and are data-limited.](../figures/calibration.png){ width=72% }
